...
def scriptasinica (target):
    ...
    for p in periods:
        ...
        try:# browser.find_element_by_xpath("//*[@name='_IMG_檢索報表']"):
            browser.find_element_by_xpath("/html/body/form/table/tbody/tr[2]/td[3]/table[2]/tbody/tr[2]/td[4]/a").click()
            #hits = browser.find_elements_by_xpath('/html/body/form/table/tbody/tr[2]/td[3]/table[2]')
            
            #for line in lines:
            #hits = browser.find_element_by_class_name("hit2")
            hits = browser.find_element_by_xpath('//element[@class="hit2"]')
...
#                if not os.path.exists('./biao/'):
#                    os.makedirs('./biao')
#                with open(f"./biao/{target}_{p['name']}.txt", mode='a', encoding='utf8') as fp:
