...
read in files, combine in 1 dataframe
```{r}
...
text_df <- map(files, read_lines) %>%
  unlist() %>%
  data_frame(text = .) %>%
  distinct() %>% # Let us also see the result without this
  # also one run with making sure that every ideophone is considered a word
...
```{r}
#write_csv(tidy_word_vectors, "./tidyhanchi_tang.csv")
```
