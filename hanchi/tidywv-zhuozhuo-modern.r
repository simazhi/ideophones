...
resulttable <- function(textobject){
  ...
  path <- sprintf("./diachroniczhuozhuo/%s%s.csv", TARGETbare, suffix)
  
  if (!file.exists(path)){
    dir.create("./diachroniczhuozhuo")
    file.create(path)
  }
...
# find files
#files <- list.files(path = "./zhuozhuo",
#                    #pattern = sprintf('*%s.txt', "_"), 
#                    full.names = TRUE
#                    )
files <- read_csv("./zhuozhuo/灼灼_modern.csv") %>%
